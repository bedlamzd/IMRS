# -*- coding: utf-8 -*-
"""Code for visualizing data in sim via pythonAPI.
...
try:
    ...
    print('eigen and sva pythonAPI modules are not available. To install run the script at:'
          ...
          'When you build the modules make sure pythonAPI bindings are enabled.')
...
def csimPrint(client_id, message):
    """Print a message in both the pythonAPI command line and on the CoppeliaSim Statusbar.
